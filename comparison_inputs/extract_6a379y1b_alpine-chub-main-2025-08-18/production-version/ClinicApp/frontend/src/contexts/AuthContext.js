...
const testAPIEndpoints = async () => {
  ...
  try {
    ...
    const response1 = await axios.post('https://mongodb-fix.preview.emergentagent.com/auth/login', 
      { test: 'data' },
  ...
  try {
    ...
    const response2 = await axios.post('https://mongodb-fix.preview.emergentagent.com/api/auth/login',
      { test: 'data' },
      { headers: { 'Content-Type': 'application/json' } }
    );
...
const getAPIBaseURL = () => {
  if (process.env.NODE_ENV === 'development') {
    ...
    const backendUrl = process.env.REACT_APP_BACKEND_URL || 'https://mongodb-fix.preview.emergentagent.com';
...
export const AuthProvider = ({ children }) => {
  ...
  //   try {await axios.post('https://mongodb-fix.preview.emergentagent.com/api/auth/login', await axios.post(`${API}/auth/login`, { username, password });
