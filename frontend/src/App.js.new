...
function App() {
  ...
  const [viewMode, setViewMode] = useState('staff'); // 'staff' or 'patient'

  const renderModule = () => {
    if (viewMode === 'patient') {
      return (
        <PatientAuthProvider>
          <PatientApp onSwitchToStaff={() => setViewMode('staff')} />
        </PatientAuthProvider>
      );
    }
    ...
    switch (activeModule) {
      ...
      case 'erx-summary':
        return (
          <ProtectedRoute>
            <AppHeader>
              <ERxSummaryView setActiveModule={setActiveModule} />
            </AppHeader>
          </ProtectedRoute>
        );
      case 'daily-log':
      ...
      default:
        return (
          <ProtectedRoute>
            <AppHeader>
              <Dashboard setActiveModule={setActiveModule} onSwitchToPatient={() => setViewMode('patient')} />
            </AppHeader>
          </ProtectedRoute>
        );
    }
