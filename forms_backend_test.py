        ...
        try:
            async with self.session.post(
                ...
                if response.status == 400:
                    ...
                    validation_errors = error_data.get("detail", {}).get("validation_errors") or error_data.get("validation_errors")
                    if validation_errors:
                        self.log_result("Validation - Data Types", True, f"Correctly rejected submission with type validation errors: {len(validation_errors)} errors")
                    else:
                        self.log_result("Validation - Data Types", False, f"Expected validation_errors in response: {error_data}")
                else:
